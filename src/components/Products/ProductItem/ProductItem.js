...
function ProductItem({ product, onDelete, onUpdate }) {
   ...
   const handleSave = async () => {
      ...
      try {
         const response = await axios.put(`http://localhost:3002/products/${product.id}/image/name/price`, formData, {
            headers: {
   ...
   return (
      <div className={cx('product-item')} key={product.id}>
         {isEditing ? (
            ...
            <>
               <img src={product.mainImage} alt={product.name} style={{ width: '100px', height: '100px' }} />
               <p>{product.name}</p>
               <p>{product.price}</p>
               ...
               <FontAwesomeIcon className={cx('edit-icon')} icon={faPenToSquare} onClick={handleEdit} />
            </>
         )}
      </div>
   );
